...
- Go Gopher：路线 id `gopher`；default=false；状态 `source-reviewed`；来源/许可证权威文件 `skills/visual-ip-illustrations/references/ips/gopher/source.md`；输出路径 `assets/<article-slug>-gopher/`；文档验证标记 `assets/&lt;article-slug&gt;-gopher/`；输出后缀 `gopher`；本地视觉权威来自根目录 `gopher.png`；署名 Renee French；许可边界 Creative Commons Attribution 4.0；公开渲染样例需要 `RELEASE_CHECKLIST.md` public-sample gate 和 Phase 42 Go Gopher release evidence；Go logo boundary 和 official endorsement boundary 随路线交付记录一起保留。
