...
当前可用执行路径包括 Xiaohei 和 Littlebox。用户省略视觉 IP 时，默认视觉 IP 使用小黑路线，沿用现有 `$ian-xiaohei-illustrations` 工作流、参考文件和输出路径。

显式选择 Xiaohei 时，可以使用这些别名：`小黑`、`Xiaohei`、`Ian`、`ian-xiaohei`。这些别名都会选择同一个 Xiaohei 路由。

Xiaohei 图片保存到 workspace 的 `assets/<article-slug>-illustrations/`。文档校验时也可按 HTML 转义形式查找 `assets/&lt;article-slug&gt;-illustrations/`。

显式选择 Littlebox 时，可以使用这些别名：`小盒`、`Littlebox`、`纸盒`、`paper-box`、`carton`。这些别名都会选择同一个 Littlebox 路由。

Littlebox 图片保存到 workspace 的 `assets/<article-slug>-littlebox/`。文档校验时也可按 HTML 转义形式查找 `assets/&lt;article-slug&gt;-littlebox/`。

同时请求多个视觉 IP 时，交付口径按 IP 分组，每个 IP 形成独立 variant group，并写入各自的输出目录。

维护者可以在 `ian-xiaohei-illustrations/references/routing.md` 查看路由元数据字段：`id`、`display_name`、`aliases`、`default`、`output_suffix`、`required_references`、`attribution_context`、`status`。Xiaohei canonical pack 位于 `ian-xiaohei-illustrations/references/ips/xiaohei/`；Littlebox canonical pack 位于 `ian-xiaohei-illustrations/references/ips/littlebox/`；根目录下的五个 `references/*.md` 文件是 legacy compatibility entry points。
