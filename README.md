...
> 为中文文章生成可选视觉 IP 的正文配图：Xiaohei 是 implicit default，可显式选择 Littlebox；Tom 是显式 protected-character route，当前状态 `gated-authorized`；Ferris 是 explicit Rust-community mascot route，当前状态 `source-reviewed`。
>
...
文档校验也保留 HTML 转义路径标记：`assets/&lt;article-slug&gt;-illustrations/`、`assets/&lt;article-slug&gt;-littlebox/`、`assets/&lt;article-slug&gt;-tom/` 和 `assets/&lt;article-slug&gt;-ferris/`。
