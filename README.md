...
当前可用执行路径是 Xiaohei。用户省略视觉 IP 时，默认视觉 IP 使用小黑路线，沿用现有 `$ian-xiaohei-illustrations` 工作流、参考文件和输出路径。
...
同时请求多个视觉 IP 时，交付口径按 IP 分组，每个 IP 形成独立 variant group。Littlebox 的执行规则、提示词、QA 和完整输出行为由 Phase 3 交付。

维护者可以在 `ian-xiaohei-illustrations/references/routing.md` 查看路由元数据字段：`id`、`display_name`、`aliases`、`default`、`output_suffix`、`required_references`、`attribution_context`、`status`。Xiaohei canonical pack 位于 `ian-xiaohei-illustrations/references/ips/xiaohei/`；根目录下的五个 `references/*.md` 文件是 legacy compatibility entry points。
