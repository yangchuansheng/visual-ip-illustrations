...
> 为中文文章生成可选视觉 IP 的正文配图：Xiaohei 是 implicit default，可显式选择 Littlebox；Tom 是显式 protected-character route，当前状态 `gated-authorized`；Ferris 是 explicit Rust-community mascot route，当前状态 `source-reviewed`；Sealos Seal 是 explicit brand-owned route under preparation。
>
...
- **Sealos Seal**：explicit `brand-owned` route under preparation。Phase 16 route-status 记录位于 `ian-xiaohei-illustrations/references/routing.md`，uploaded-image authority 位于 `ian-xiaohei-illustrations/references/ips/sealos/source.md`。
...
Sealos Seal is an explicit `brand-owned` route under preparation for the uploaded Sealos white seal mascot.

可用别名：`Sealos Seal`、`Sealos mascot`、`Sealos 吉祥物`、`Sealos 海豹`、`white Sealos seal`、`blue hoodie seal`。

Uploaded-image authority：`ian-xiaohei-illustrations/references/ips/sealos/source.md`。Route metadata：`ian-xiaohei-illustrations/references/routing.md`。

Brand context：reliable cloud developer companion tied to Sealos cloud OS, AI-native deployment, DevBox, databases, app deployment, and Kubernetes. Prior Sealos mascot exploration supplies supporting brand context; the uploaded image is the v1.3 shape authority.

输出路径合同：`assets/<article-slug>-sealos/`。文档校验 token：`assets/&lt;article-slug&gt;-sealos/`。输出后缀：`sealos`。

Public rendered Sealos samples require release review for uploaded-image identity drift, cap/hoodie/logo preservation, generic seal drift, abstract logo creature drift, and prior-variant drift.
...
- Sealos Seal: `ian-xiaohei-illustrations/references/ips/sealos/`，uploaded-image authority `ian-xiaohei-illustrations/references/ips/sealos/source.md`

同时请求多个视觉 IP 时，交付口径按 IP 分组，每个 IP 形成独立 variant group，并写入各自输出目录。Xiaohei 是 implicit default route；Littlebox 是 explicit active route；Tom 是 explicit protected-character route，当前 status 为 `gated-authorized`；Ferris 是 explicit Rust-community mascot route，当前 status 为 `source-reviewed`；Sealos Seal 是 explicit brand-owned route under preparation。
...
```text
Use $ian-xiaohei-illustrations 先不要生图。
请用同一个 core idea 分别输出 Xiaohei、Littlebox、Tom 和 Ferris 四个 separate variant group。
...
2. 选择视觉 IP：省略时走 Xiaohei，显式 Littlebox 时走 Littlebox，显式 Tom aliases 时走 Tom protected-character route，显式 Ferris aliases 时走 Ferris source-reviewed pack
3. 提炼核心观点、认知转折、流程结构和适合视觉化的段落
4. 先输出 shot list：每张图只选一个认知锚点
5. 为每张图选择结构类型：Workflow、系统局部、前后对比、角色状态、概念隐喻、方法分层、地图路线或小漫画分镜
6. 按所选 IP 读取 canonical pack，组装提示词并单张生成；混合 IP 请求生成 separate route groups 和独立输出目录，Xiaohei、Littlebox、Tom 和 Ferris 各自保留 route-local references
7. 按所选 IP 的 QA checklist 检查角色身份、构图、背景、标注和输出路径；Tom 路由保留 `gated-authorized` 与 `ian-xiaohei-illustrations/references/ips/tom/rights.md`，Ferris 路由保留 `source-reviewed`、Source/trademark note 与 `ian-xiaohei-illustrations/references/ips/ferris/source.md`
8. 保存最终 PNG，并报告用途和路径
...
验证范围包括 skill 包形状、路由表、Xiaohei、Littlebox、Tom 和 Ferris canonical packs、legacy Xiaohei 路径、公开文档链接、输出路径标记、NOTICE 归因、Tom `gated-authorized` route markers、Ferris `source-reviewed` route markers 和 Source/trademark note。

Phase 15 负责 validator/test hardening 和 green Node evidence，并用同一 change set 运行：
