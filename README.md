...
> 为中文文章生成可选视觉 IP 的正文配图：Xiaohei 是 implicit default，可显式选择 Littlebox；Tom 是显式 protected-character route，当前状态 `gated-authorized`。
>
...
同时请求多个视觉 IP 时，交付口径按 IP 分组，每个 IP 形成独立 variant group，并写入各自输出目录。Xiaohei 是 implicit default route；Littlebox 是 explicit active route；Tom 是 explicit protected-character route，当前 status 为 `gated-authorized`。
