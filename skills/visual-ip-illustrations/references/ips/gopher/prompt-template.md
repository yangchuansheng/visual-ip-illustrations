# Go Gopher Prompt Template

Use this file after selecting the `gopher` route. Runtime planning prose may follow the user's language. Image-generation prompts stay English for model consistency. Visible labels are copied exactly in the user's requested language.
...
Use this format for Go Gopher shot lists:
...
1. Placement: [where this image appears in the article]
   ...
   Gopher state: [focused / inspecting / pointing / carrying / sorting / bridging / repairing / guiding / stamping / marking / tuning / shielding / weighing / connecting / untangling / mapping / comparing / lifting / assembling / routing]
   Gopher action: [the physical cognitive action Go Gopher performs]
...
Gopher action: Go Gopher must perform the central cognitive article action: [specific action]. The metaphor should lose its meaning if Go Gopher is removed.

Supporting objects: [3-6 sparse physical article metaphor objects: maps, bridges, knots, compasses, shelves, lamps, shields, stamps, keys, trails, gates, scales, buckets, envelopes, threads, lenses, hooks, stepping stones, signposts, levers, or small hand-built machines].

Composition: one core idea only. [Describe exact positions, Go Gopher's body direction, paw action, eye direction, the supporting objects, and the information flow]. Main Go Gopher action takes about 35%-55% of the canvas. Leave generous whitespace around the action. Keep labels away from Go Gopher's ears, large eyes, black nose, buck teeth, beige muzzle and paws, compact limbs, black outline, and white sticker-like border.

Article metaphor: use physical objects as explanatory metaphors for trust, proof, review, dependency, handoff, hidden cost, failure mode, decision gate, system resilience, judgment, sequencing, or conceptual repair. Go-themed props may appear only as article metaphors when they preserve Go Gopher as the character route and avoid Go logo identity or official endorsement cues.
...
Save reminder: save accepted output under `assets/<article-slug>-gopher/` with an ordered English slug filename such as `01-topic-name.png`.

Constraints: one image explains one core idea. Keep Go Gopher as the central cognitive action subject. Use no Go logo identity, no Go-logo-forward composition, no official Go project affiliation cues, no approval claims, no sponsorship claims, no endorsement wording, no public mascot asset-sheet layout, no generic blue mascot drift, no realistic animal drift, no missing ears, no missing large simple eyes, no missing buck teeth, no missing beige muzzle/paws, no missing black outline, no missing white sticker-like border, no passive placement, no formal diagrams, no dense text, no code screenshots, no IDE scenes, no terminal scenes, no UI screenshots, no top-left titles, no clean digital font labels, no crowded nodes, no route leakage, and no copied prior composition.
...
Each edit prompt keeps the Go Gopher route status and source/license boundary explicit. Preserve the existing composition, labels, style, aspect ratio, and unaffected objects except where the named failure requires a focused repair.

### Stronger Go Gopher Participation

Stronger Go Gopher Participation repair gate.
