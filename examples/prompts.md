...
帽子、胸前、吉祥物身体、道具和场景保持无 logo：plain navy cap with no logo、plain deep-blue hoodie chest with no logo、no cap logo、no chest logo、no mascot logos、no logo patches、no logo-like wave/cloud mark、no emblem、no text badge。
保存路径使用 `assets/<article-slug>-sealos/`；文档校验 token 是 `assets/&lt;article-slug&gt;-sealos/`。
交付报告保留 Brand/canonical-image note，并说明 public rendered Sealos samples 以 RELEASE_CHECKLIST.md 的 uploaded-image identity、no-logo mascot identity 和 Sealos brand wording gate 为准。
...
预期：uploaded-image identity markers 包含 white rounded seal body、plain navy cap with no logo、plain deep-blue hoodie chest with no logo、glossy dark eyes、black nose、whisker dots、small smile、short rounded flippers、compact legs、side-rear white tail。
预期：no-logo markers 包含 no cap logo、no chest logo、no mascot logos、no logo patches、no logo-like wave/cloud mark、no emblem、no text badge。
预期：public sample review blocks generic seal, abstract logo creature, logo-bearing mascot variants, missing cap, missing hoodie, cap logo, chest logo, mascot logo, logo patch, logo-like wave/cloud mark, generated logo approximation, generated logo tracing, redrawn logo, alternate wave mark, simplified logo mark, missing side-rear tail, plain bald-head, prior exploration variant, and different selected mascot drift。
...
- Public rendered Sealos samples 由 RELEASE_CHECKLIST.md 的 uploaded-image identity、no-logo mascot identity 和 Sealos brand wording gate 控制
