# Sealos Seal Style DNA

Route id: `sealos`.
Route status: `brand-owned`.
Output path: `assets/<article-slug>-sealos/`.
Brand/canonical-image authority: `source.md`.
Canonical image status: `uploaded-image-canonical`.
Drift boundary: `uploaded-image-locked`.
Public rendered Sealos samples remain gated by `RELEASE_CHECKLIST.md`.
Sealos route block: generic seal drift, abstract logo creature drift, missing cap, missing hoodie, missing Sealos marks, changed body color, passive mascot placement, over-detailed 3D toy drift, excessive text, and route leakage all fail the route.
Save accepted Sealos output under `assets/<article-slug>-sealos/` with an ordered English slug filename such as `01-topic-name.png`.

## Core Style

Sealos Seal images are sparse 16:9 horizontal article illustration outputs where the uploaded white Sealos Seal mascot performs one cognitive action inside an original cloud-developer article metaphor.

## Must Have

- 16:9 horizontal article illustration by default.
- Clean white or near-white background with generous whitespace.
- One core idea per image.
- The uploaded Sealos Seal mascot performs the core conceptual action.
- Loose black linework with visible hand-drawn looseness.
- Restrained Sealos blue and deep-blue accents for the cap, hoodie, infrastructure cues, and selected action emphasis.
- Minimal red/orange emphasis for warnings, decisions, or key transitions.
- Concise visible labels, usually 2-6 total, copied in the user's language.
- Sparse supporting objects selected for the article idea.
- Sealos Seal recognition stays clear inside the simplified drawing style.

## Sealos Identity Markers

Preserve the uploaded-image markers together:

- white rounded seal body
- navy cap
- deep-blue hoodie
- Sealos logo marks on cap and chest
- glossy dark eyes
- black nose
- whisker dots
- small smile
- short rounded flippers
- compact legs
- side-rear white tail

The uploaded-image identity matters more than rendering detail density. Keep the mascot simple enough for article explanation while preserving the cap, hoodie, Sealos marks, body color, face, flippers, legs, and tail.

## Composition

- Use a single visual structure: Workflow, System Local View, Before/After, Character State, Concept Metaphor, Method Layers, Map Route, or Mini Comic.
- Make the mascot action physically readable: deploy, launch, connect, debug, route, lift, inspect, repair, shield, bridge, tune, roll back, stamp, sort, assemble, or guide.
- Use low-tech cloud-developer metaphor props sparingly: cloud OS tiles, deployment rails, DevBox workbenches, database cylinders, Kubernetes pods, app launch pads, traffic routes, incident cones, log slips, config keys, health checks, rollback handles, workflow gates, or service bridges.
- Treat cloud-developer props as article metaphors for launch, reliability, review, diagnosis, handoff, hidden coupling, failure mode, decision gates, or system resilience.
- Leave empty space around the mascot action so the metaphor reads at a glance.
- Keep labels short and separated from the mascot's face, cap, chest logo, hoodie, flippers, and tail.
- Sealos cognitive-action participation gate: the Sealos Seal performs the core cognitive action, so the image loses its meaning if the mascot is removed.

## Sealos Visual Vetoes

Regenerate or edit when the result contains:

- generic seal drift
- abstract logo creature drift
- missing cap
- missing hoodie
- missing Sealos marks
- changed body color
- passive mascot placement
- over-detailed 3D toy drift
- excessive text
- route leakage
- over-detailed 3D toy rendering
- character-sheet rendering
- plush-toy rendering
- polished vector mascot rendering
- generic mascot rendering
- formal diagrams
- dense PPT-like infographics
- UI screenshots
- IDE or terminal scenes
- code screenshots
- poster layouts
- top-left title artifacts
- dense text blocks
- clean digital labels
- copied prior route compositions
- prior exploration variant drift
- bald/plain-head seal drift

## Stable Gates

- Sealos identity gate: the image preserves white rounded seal body, navy cap, deep-blue hoodie, Sealos logo marks on cap and chest, glossy dark eyes, black nose, whisker dots, small smile, short rounded flippers, compact legs, and side-rear white tail.
- Sealos cognitive-action participation gate: the Sealos Seal performs the central cognitive action, and removing the mascot breaks the metaphor.
- Sealos uploaded-image gate: the mascot follows `source.md`, `uploaded-image-canonical`, and `uploaded-image-locked`.
- Sealos sparse article gate: the scene stays clean white or near-white, hand-drawn, low-density, and label-light.
- Sealos route block: generic seal drift, abstract logo creature drift, missing cap, missing hoodie, missing Sealos marks, changed body color, passive mascot placement, over-detailed 3D toy drift, excessive text, and route leakage all fail the route.
