...
- Source context: OpenClaw personal AI assistant logo-mascot route for article illustrations.
...
- OpenClaw is a source-reviewed logo-mascot route for sparse 16:9 article illustrations.
- OpenClaw should carry cognitive article actions as a red logo-derived mascot subject.
- OpenClaw route references stay separate from Xiaohei, Littlebox, Tom, Ferris, and Seal route references.
- OpenClaw outputs belong under `assets/<article-slug>-openclaw/`.

## Uploaded Logo Visual Markers

Preserve these uploaded OpenClaw logo markers together:
...
- Owner: maintainer / release reviewer for the OpenClaw route.
- Review purpose: decide whether OpenClaw remains `source-reviewed`, receives public rendered samples, or receives a different release status after downstream phases.
- Required review fields: reviewer, review date, approval status, source status, license boundary, uploaded-logo identity outcome, route isolation outcome, allowed use, restricted use, distribution channels, and public-sample decision.
