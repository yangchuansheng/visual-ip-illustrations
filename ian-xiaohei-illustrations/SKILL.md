...
- 小黑路由沿用现有 `$ian-xiaohei-illustrations` 行为、所需参考文件和 `assets/<article-slug>-illustrations/` 输出路径。
...
小黑路由继续使用当前小黑参考：`references/style-dna.md`、`references/xiaohei-ip.md`、`references/composition-patterns.md`、`references/prompt-template.md` 和 `references/qa-checklist.md`。小黑提示词继续包含：
...
Phase-owned routes use their own `required_references` and route-owned prompt rules. 案例只提供风格密度和角色参与方式，生成时从当前文章重新发明一个奇怪但成立的隐喻。

### 5. 检查与迭代

生成后检查所选 IP 的 QA 参考。小黑路由使用 `references/qa-checklist.md`。如果出现以下问题，优先重生成或局部编辑：
